import { getOrCreateCategory } from '../parser.js'
...
export async function dashboardSummary(db, userId, today) {
...
export async function libraryTree(db, userId) {
...
export async function topicDetail(db, userId, topicId) {
...
export async function dailyPlan(db, userId, planDate) {
...
export async function listMcqs(db, userId) {
  const topics = db.find('topics', { user_id: userId })
  const result = []
...
export async function saveParsedDocument(db, userId, parsed, filename) {
