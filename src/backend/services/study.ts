import { getOrCreateCategory } from '../parser'
import type { ParsedResult } from '../parser'

function parseOptions(value: string): any[] {
  try {
...
interface DbLike {
  ...
  update(table: string, id: number, updates: Record<string, any>): Promise<boolean>
  delete(table: string, id: number): Promise<void>
  deleteWhere(table: string, filters?: Record<string, any>): Promise<void>
  count(table: string, filters?: Record<string, any>): number
}

async function dashboardSummary(db: DbLike, userId: number, today: string): Promise<any> {
  ...
  if (topicIds.length === 0) {
    ...
    allTopics.sort((a: any, b: any) => (b.id || 0) - (a.id || 0))
    topicIds = allTopics.slice(0, 5).map((t: any) => t.id)
  }
  ...
  const pendingTodos = todos.filter((t: any) => !t.is_completed).length
  ...
  const userTopicIds = new Set(db.find('topics', { user_id: userId }).map((t: any) => t.id))
  let totalReading = 0
  let totalMcqs = 0
  let totalTodos = 0
...
async function libraryTree(db: DbLike, userId: number): Promise<any[]> {
  ...
  function rollup(cat: any, visited: Set<number> = new Set()): number {
...
async function topicDetail(db: DbLike, userId: number, topicId: number): Promise<any> {
  ...
  readings.sort((a: any, b: any) => (a.order_idx || 0) - (b.order_idx || 0))

  const formulas = db.find('formulas', { topic_id: topicId })
  formulas.sort((a: any, b: any) => (a.order_idx || 0) - (b.order_idx || 0))

  const notes = db.find('notes', { topic_id: topicId })
  notes.sort((a: any, b: any) => (a.order_idx || 0) - (b.order_idx || 0))

  const mcqRows = db.find('mcqs', { topic_id: topicId })
  mcqRows.sort((a: any, b: any) => (a.order_idx || 0) - (b.order_idx || 0))

  const questions = mcqRows.map((row: any) => ({
    id: row.id,
  ...
  const counts: Record<string, number> = { reading: readings.length, formulas: formulas.length, notes: notes.length, mcqs: questions.length }
...
async function dailyPlan(db: DbLike, userId: number, planDate: string): Promise<any> {
  const plans = db.find('daily_plans', { user_id: userId, plan_date: planDate })
  const result: any = { topics: [], reading: [], formulas: [], notes: [], mcqs: [], todos: [] }
...
async function listMcqs(db: DbLike, userId: number): Promise<any[]> {
  const topics = db.find('topics', { user_id: userId })
  ...
  const result: any[] = []
  const userTopicIds = new Set(topics.map((t: any) => t.id))
  for (const m of db.find('mcqs')) {
  ...
  result.sort((a: any, b: any) => (a.topic_title || '').localeCompare(b.topic_title || '') || (a.id || 0) - (b.id || 0))
  return result
}
...
async function saveParsedDocument(db: DbLike, userId: number, parsed: ParsedResult, filename: string): Promise<number> {
  ...
  if (meta.subcategory) {
    ...
    categoryId = await getOrCreateCategory(db as any, meta.category, null, userId)
  }
...
export { parseOptions, dashboardSummary, libraryTree, topicDetail, dailyPlan, listMcqs, saveParsedDocument }
